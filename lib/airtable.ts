...
async function airtableFetch(table, params = {}, revalidate = 3600) {
...
function normalise(record) {
  ...
  return {
    ...
    seasons: f['Seasons'] ?? [],
    photos: heroAtt ? [{ url: heroAtt.url, thumbUrl: heroAtt.thumbnails?.large?.url ?? heroAtt.url, attribution: heroAtt.filename, license: 'CC-BY', source: 'airtable' }] : [],
  };
}
...
export async function getAllSpeciesSlugs() {
  const all = await getAllSpecies();
  return all.map(s => s.slug).filter(Boolean);
}

export async function getFeaturedSpecies(limit = 3) {
  const all = await getAllSpecies();
